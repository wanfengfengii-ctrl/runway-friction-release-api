...
连续巡检车获得不等距测点后，可改走**剖面评估入口**：提交跑道长度、固定检查
窗长与 6–50 个里程—摩阻测点，服务在相邻测点间线性插值、以分段梯形积分建立
前缀面积，在整条跑道上定位平均摩阻最低的定长连续窗口（候选不限于测点起点，
还包括窗口两端插值相等的内部驻点），按未舍入的平均值沿用 0.40 / 0.30 边界判级。

- **运行时**：Python 3.12 · FastAPI · Pydantic v2
- **测试**：pytest（124 项，覆盖输入校验、分段判级、五点稳健采样、等权相邻违序合并、
  校准评估、剖面评估（内部驻点最优窗、并列取最早、临界均值判级）与 422 整份拒绝）
...
## 连续巡检剖面评估（`/api/v1/friction/profile-assess`）

连续巡检车获得不等距测点后，维护人员可提交跑道编号、跑道长度、固定检查窗长
与 6–50 个里程—摩阻测点，查看整条跑道上平均摩阻最低的定长连续窗口。
服务在相邻测点间作线性插值，以分段梯形积分建立前缀面积，再按测点里程及其
减去窗长的位置切分窗口起点域；每个区间检查两端与满足窗口两端插值相等的
内部驻点（不枚举测点起点），全局取窗口平均值最小者，并列时取起点最小者。
计算全程使用 `Decimal`，响应数值保留六位小数；平均值按未舍入值沿用
0.40 / 0.30 边界判级（≥0.40 正常、0.30–<0.40 关注、<0.30 关闭）。
...
| `runway_length` | number | 跑道长度（与里程同单位，大于 0） |
| `window_length` | number | 固定检查窗长度（大于 0 且小于跑道长度） |
| `points` | object[] | 6–50 个测点，每项含 `mileage`（里程）与 `coefficient`（摩阻系数） |

测点约束：里程严格递增（不得重复或倒退）、恰好覆盖零点与跑道终点
（首点里程为 0、末点里程等于跑道长度）、不得越界；系数 0.00–1.00、
至多两位小数；非有限数、字符串、布尔值、超大整数均以 422 整份拒绝，
错误位置 `loc` 指向对应字段。
...
| --- | --- |
| `runway_id` | 跑道编号回显 |
| `start_mileage` / `end_mileage` | 最低平均窗的起止里程 |
| `start_friction` / `end_friction` | 窗口两端的插值摩阻 |
| `area` | 窗口积分面积（梯形法） |
| `average` | 窗口平均摩阻（判级按未舍入值） |
| `rating` | 窗口平均值判级（正常 / 关注 / 关闭） |

### 剖面评估请求示例（最低窗落在内部驻点）

```bash
curl -X POST http://localhost:8000/api/v1/friction/profile-assess \
  -H 'Content-Type: application/json' \
  -d '{
    "runway_id": "18L",
    "runway_length": 400,
    "window_length": 180,
    "points": [
      {"mileage": 0,   "coefficient": 0.80},
      {"mileage": 50,  "coefficient": 0.65"},
      {"mileage": 100, "coefficient": 0.50},
      {"mileage": 150, "coefficient": 0.35},
      {"mileage": 200, "coefficient": 0.20},
      {"mileage": 250, "coefficient": 0.35"},
      {"mileage": 300, "coefficient": 0.50},
      {"mileage": 350, "coefficient": 0.65},
      {"mileage": 400, "coefficient": 0.80}
    ]
  }'
```

V 形剖面（中间点为共线插值点）上，最低平均窗落在内部驻点 110–290
（两端插值均为 0.47），平均值 0.335，严格优于所有测点起点与测点减窗长
断点候选（最优断点 0.336667）。响应：
...
### `POST /api/v1/friction/profile-assess`

连续巡检剖面评估入口：维护人员提交跑道编号、跑道长度、固定检查窗长与 6–50 个
里程—摩阻测点（里程严格递增并恰好覆盖零点与跑道终点），服务在相邻测点间线性
插值，以分段梯形积分建立前缀面积，在整条跑道上定位平均摩阻最低的定长连续窗口
（候选不限于测点起点，含窗口两端插值相等的内部驻点；并列时取起点最小者），
返回窗口起止里程、两端插值、积分面积与平均值（数值保留六位小数），并按未舍入
平均值沿用 0.40 / 0.30 边界判级。详见上文「连续巡检剖面评估」一节。
